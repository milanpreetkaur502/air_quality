/**
  ******************************************************************************
  * @file    subscribe_publish_sensor_values_patch.c
...
#endif
if ((checkIoTDeviceConfig() != 0) || dialog_ask("\nDo you want to update your IoT device credentials? [y/n]:"))
  {
...
void cloud_run(void const *arg)
{
...
#ifdef  AWS_IOT_DASHBOARD
  snprintf(cPTopicName_data, sizeof(cPTopicName_data), "atr/v1/%s/telemetry",deviceName );
  ...
  printf("Press the User button (Blue) to publish the LED desired value on the %s topic\n", cPTopicName);
	...
	osDelay(1000);	// Let TeseoConsumerTask start
	if (!use_stored_coordinates)
	{
		printf("Waiting for GNSS fix (push user button to skip)");
