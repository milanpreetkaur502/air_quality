...
static void MainThread(void const * argument)
{
  osThreadId teseoConsumerTaskHandle = NULL;
  ...
  osThreadDef(teseoConsumerTask, TeseoConsumerTask, osPriorityNormal, 0, 1024);
  ...
  if ( (CloudThreadId == NULL) || (teseoConsumerTaskHandle == NULL) )
  {
     Error_Handler();
  }
#if (STACK_ANALYSIS_TRACE == 1)
  else
  {
     ...
     stackAnalysis_addStackSizeByHandle(teseoConsumerTaskHandle, 1024);
  }
#endif
...
/**	
* @brief  This function creates the Mutex for GNSS Data access
* @param  None
* @retval None
*/
static void GNSSData_Mutex_Init(void)
